...
public class BlockFire extends Block
{
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos)
    {
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down()))
        {
            ...
            return state.withProperty(NORTH, this.canCatchFire(worldIn, pos.north()))
            		...
            		.withProperty(UPPER, l).withProperty(FLIP, flag1).withProperty(ALT, flag);
        }
        else
    ...
    {
        super(Material.fire);
        this.setDefaultState(this.blockState.getBaseState().withProperty(AGE, 0).withProperty(FLIP, false).withProperty(ALT, false)
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (worldIn.getGameRules().getBoolean("doFireTick"))
        {
            ...
            else
            {
                ...
                if (i < 15)
                {
                    state = state.withProperty(AGE, i + (rand.nextInt(3) >> 1));
    ...
    {
        if (worldIn.provider.getDimensionId() > 0 || !Blocks.portal.createPortal(worldIn, pos))
        {
            if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !this.canNeighborCatchFire(worldIn, pos))
                worldIn.setBlockToAir(pos);
            else
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down()))
        {
            if (Blocks.fire.canCatchFire(worldIn, pos.west()))
                for (int j = 0; j < 2; ++j)
                {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d3, d8, d13, 0, 0, 0);
                }
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.east()))
                for (int k = 0; k < 2; ++k)
                {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d4, d9, d14, 0, 0, 0);
                }
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.north()))
                for (int l = 0; l < 2; ++l)
                {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d5, d10, d15, 0, 0, 0);
                }
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.south()))
                for (int i1 = 0; i1 < 2; ++i1)
                {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d6, d11, d16, 0, 0, 0);
                }
            ...
            if (Blocks.fire.canCatchFire(worldIn, pos.up()))
                for (int j1 = 0; j1 < 2; ++j1)
                {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d7, d12, d17, 0, 0, 0);
                }
        }
        else
            for (int i = 0; i < 3; ++i)
            {
                ...
                worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d0, d1, d2, 0, 0, 0);
            }
    }

...
public class BlockDynamicLiquid extends BlockLiquid
{
    ...
    private int calculateFlowCost(World worldIn, BlockPos pos, int distance, EnumFacing facing)
    {
        int i = 1000;
        ...
        for (EnumFacing enumfacing : Plane.HORIZONTAL)
            if (enumfacing != facing)
            {
                ...
                if (!this.isBlocked(worldIn, blockpos, iblockstate) && (iblockstate.getBlock().getMaterial() != this.blockMaterial || iblockstate.getValue(LEVEL) > 0))
                {
                    ...
                    if (distance < 4)
                    {
                        int j = this.calculateFlowCost(worldIn, blockpos, distance + 1, enumfacing.getOpposite());
    ...
    private Set<EnumFacing> getPossibleFlowDirections(World worldIn, BlockPos pos)
    {
        ...
        for (EnumFacing enumfacing : Plane.HORIZONTAL)
        {
            ...
            if (!this.isBlocked(worldIn, blockpos, iblockstate) && (iblockstate.getBlock().getMaterial() != this.blockMaterial || iblockstate.getValue(LEVEL) > 0))
            {
                int j = this.isBlocked(worldIn, blockpos.down(), worldIn.getBlockState(blockpos.down())) ? 
                		this.calculateFlowCost(worldIn, blockpos, 1, enumfacing.getOpposite()) : 0;

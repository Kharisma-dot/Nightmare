...
public class BlockRedstoneWire extends Block
{
    ...
    private EnumAttachPosition getAttachPosition(IBlockAccess worldIn, BlockPos pos, EnumFacing direction)
    {
        		...
        		&& (block.isBlockNormalCube() || !canConnectUpwardsTo(worldIn.getBlockState(blockpos.down()))))
        {
            ...
            return !block1.isBlockNormalCube() && block.isBlockNormalCube() 
            		&& canConnectUpwardsTo(worldIn.getBlockState(blockpos.up())) ? EnumAttachPosition.UP : EnumAttachPosition.NONE;
        }
        else
    ...
    public int colorMultiplier(IBlockAccess worldIn, BlockPos pos, int renderPass)
    {
        ...
        return iblockstate.getBlock() != this ? 
        		super.colorMultiplier(worldIn, pos, renderPass) : this.colorMultiplier(iblockstate.getValue(POWER));
    }
    ...
    public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state)
    {
        if (!worldIn.isRemote)
        {
            ...
            for (EnumFacing enumfacing2 : Plane.HORIZONTAL)
            {
        ...
        else
        {
            ...
            else
            {
                ...
                for (EnumFacing enumfacing : Plane.HORIZONTAL)
                    if (this.canConnect(worldIn, pos, enumfacing))
    ...
    private boolean canConnect(IBlockAccess worldIn, BlockPos pos, EnumFacing side)
    {
        ...
        return !flag1 && flag && canConnectUpwardsTo(worldIn, blockpos.up()) ?
        		true : (canConnectTo(iblockstate, side) ?
        		true : (block == Blocks.powered_repeater && iblockstate.getValue(BlockRedstoneDiode.FACING) == side ?
        		true : !flag && canConnectUpwardsTo(worldIn, blockpos.down())));
    }
    ...
    private int colorMultiplier(int powerLevel)
    {
        ...
        float f1 = powerLevel == 0 ? 0.3f : f * 0.6f + 0.4f;

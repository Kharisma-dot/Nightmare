...
public class BlockPumpkin extends BlockDirectional
{
    ...
    private static final Predicate<IBlockState> summonState = (state) -> 
    	state != null && (state.getBlock() == Blocks.pumpkin || state.getBlock() == Blocks.lit_pumpkin);
    ...
    private void trySpawnGolem(World worldIn, BlockPos pos)
    {
        ...
        if ((blockpattern$patternhelper = this.getSnowmanPattern().match(worldIn, pos)) != null)
        {
            ...
            entitysnowman.setLocationAndAngles((double)blockpos1.getX() + 0.5D, (double)blockpos1.getY() + 0.05D, (double)blockpos1.getZ() + 0.5D,
            		0f, 0f);
            worldIn.spawnEntityInWorld(entitysnowman);

            for (int j = 0; j < 120; ++j)
                worldIn.spawnParticle(EnumParticleTypes.SNOW_SHOVEL, (double)blockpos1.getX() + worldIn.rand.nextDouble(),
                		(double)blockpos1.getY() + worldIn.rand.nextDouble() * 2.5D, (double)blockpos1.getZ() + worldIn.rand.nextDouble(), 0, 0, 0);
        ...
        else if ((blockpattern$patternhelper = this.getGolemPattern().match(worldIn, pos)) != null)
        {
            ...
            entityirongolem.setLocationAndAngles((double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 0.05D, (double)blockpos.getZ() + 0.5D,
            		0f, 0f);
            ...
            for (int j1 = 0; j1 < 120; ++j1)
                worldIn.spawnParticle(EnumParticleTypes.SNOWBALL, (double)blockpos.getX() + worldIn.rand.nextDouble(),
                		(double)blockpos.getY() + worldIn.rand.nextDouble() * 3.9D, (double)blockpos.getZ() + worldIn.rand.nextDouble(), 0, 0, 0);
    ...
    protected BlockPattern getSnowmanPattern()
    {
        if (this.snowmanPattern == null)
            ...
            .where('^', BlockWorldState.hasState(summonState)).where('#', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.snow))).build();
    ...
    protected BlockPattern getGolemPattern()
    {
        if (this.golemPattern == null)
            ...
            .where('^', BlockWorldState.hasState(summonState))
            .where('#', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.iron_block)))
            .where('~', BlockWorldState.hasState(BlockStateHelper.forBlock(Blocks.air))).build();

...
public class GameSettings
{
    ...
    public boolean enumFloatSettings = true;
    ...
    {
        return key < 0 ? I18n.format("key.mouseButton", new Object[] {
        		key + 101}): (key < 256 ? Keyboard.getKeyName(key) : String.format("%c", 
        				new Object[] {(char)(key - 256)}).toUpperCase());
    }
    ...
    public void setOptionValue(GameSettings.Options settingsOption, int value)
    {
        ...
        {
            this.enumFloatSettings = !this.enumFloatSettings;
        }
    ...
    public String getKeyBinding(GameSettings.Options settingOption)
    {
        ...
        else
        {
            ...
            if (settingOption.getEnumFloat())
            {
                ...
                return settingOption == GameSettings.Options.MIPMAP_LEVELS && (double)f1 >= 4.0D ?
                		s1 + Lang.get("of.general.max") : (settingOption == GameSettings.Options.SENSITIVITY ?
                				(f == 0.0F ? s1 + I18n.format("options.sensitivity.min") : (f == 1.0F ?
                		s1 + I18n.format("options.sensitivity.max") : s1 + (int)(f * 200.0F) + "%")) :
                		(settingOption == GameSettings.Options.FOV ? (f1 == 70.0F ? s1 + I18n.format("options.fov.min") :
                		(f1 == 110.0F ? s1 + I18n.format("options.fov.max") : s1 + (int)f1)) :
                		(settingOption == GameSettings.Options.FRAMERATE_LIMIT ? (f1 == settingOption.$VALUES ?
                			s1 + I18n.format("options.framerateLimit.max") : s1 + (int)f1 + " fps") :
                		(settingOption == GameSettings.Options.RENDER_CLOUDS ?
                		(f1 == settingOption.valueMax ? s1 + I18n.format("options.cloudHeight.min") :
                		s1 + ((int)f1 + 128)) : (settingOption == GameSettings.Options.GAMMA ? (f == 0.0F ?
                			s1 + I18n.format("options.gamma.min") : (f == 1.0F ?
                		s1 + I18n.format("options.gamma.max") : s1 + "+" + (int)(f * 100.0F) + "%")) :
                		...
                		(settingOption == GameSettings.Options.MIPMAP_LEVELS ? (f1 == 0.0F ? s1 + I18n.format("options.off") :
                		s1 + (int)f1) : (f == 0.0F ? s1 + I18n.format("options.off") :
                		s1 + (int)(f * 100.0F) + "%")))))))))))));
            }
            ...
            {
                return s1 + getTranslation(AMBIENT_OCCLUSIONS, this.particleSetting);
            }
                ...
                if (this.bc.exists())
                {
                    ...
                    while ((s = bufferedreader.readLine()) != null)
                    {
                        try
                        {
                            ...
                            {
                                this.enumFloatSettings = astring[1].equals("true");
                            }
    ...
    public void saveOptions()
    {
        try
        {
            ...
            printwriter.println("realmsNotifications:" + this.enumFloatSettings);

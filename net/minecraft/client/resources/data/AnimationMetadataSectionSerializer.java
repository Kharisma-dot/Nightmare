...
public class AnimationMetadataSectionSerializer extends BaseMetadataSectionSerializer<AnimationMetadataSection> implements JsonSerializer<AnimationMetadataSection>
{
    ...
    public JsonElement serialize(AnimationMetadataSection meta, Type type, JsonSerializationContext context)
    {
        ...
        jsonobject.addProperty("frametime", meta.getFrameTime());

        if (meta.getFrameWidth() != -1)
        {
            jsonobject.addProperty("width", meta.getFrameWidth());
        }

        if (meta.getFrameHeight() != -1)
        {
            jsonobject.addProperty("height", meta.getFrameHeight());
        }
        ...
        if (meta.getFrameCount() > 0)
        {
            ...
            for (int i = 0; i < meta.getFrameCount(); ++i)
            {
                if (meta.frameHasTime(i))
                {
                    ...
                    jsonobject1.addProperty("index", meta.getFrameIndex(i));
                    jsonobject1.addProperty("time", meta.getFrameTimeSingle(i));
                    jsonarray.add(jsonobject1);
                }
                else
                {
                    jsonarray.add(new JsonPrimitive(meta.getFrameIndex(i)));
                }
            }

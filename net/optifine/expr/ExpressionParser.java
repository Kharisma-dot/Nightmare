...
public class ExpressionParser
{
    ...
    private static IExpression makeFunction(FunctionType type, IExpression[] args) throws ParseException
    {
        ...
        if (args.length != aexpressiontype.length)
            throw new ParseException("Invalid number of arguments, function: \"" + type.getName() + 
            		"\", count arguments: " + args.length + ", should be: " + aexpressiontype.length);
        else
        {
            for (int i = 0; i < args.length; ++i)
            {
                ...
                if (expressiontype != expressiontype1)
                    throw new ParseException("Invalid argument type, function: \"" + type.getName() + 
                    		"\", index: " + i + ", type: " + expressiontype + ", should be: " + expressiontype1);
            }
            
            switch(type.getExpressionType()) {
            case FLOAT:
    ...
    private static Deque<Token> getGroup(Deque<Token> deque, TokenType tokenTypeEnd, boolean tokenEndRequired) throws ParseException
    {
        ...
        while (iterator.hasNext())
        {
            ...
            else if (token.getType() == TokenType.BRACKET_CLOSE)
                --i;
        }
